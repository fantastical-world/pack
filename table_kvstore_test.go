...
func Test_MessupDatabaseErrors(t *testing.T) {
	...
	err = os.Remove("./messedup.db")
	if err != nil {
		t.Errorf("unexpected err encountered, %s", err)
	}
}

func setupBadDatabase() error {
	db, err := bolt.Open("./messedup.db", 0600, &bolt.Options{Timeout: time.Second * 10})
	if err != nil {
		return err
	}
	defer db.Close()
	err = db.Update(func(tx *bolt.Tx) error {
		...
		err = b.Put([]byte("notatable"), encoded)
		if err != nil {
			return err
		}
